...
> 2026-08-19 全面评测：**824 条查询 / 155 文档 / 4194 块**，语料主力为权威公开数据——DuReader-retrieval（百度 ACL 2022）、CMRC 2018（哈工大讯飞人工标注）、中文维基完整长条目；负样本含对抗性陷阱。全流程可复现，完整报告见 [`docs/eval_report_20260819.md`](docs/eval_report_20260819.md)。

| 维度 | 指标 | 结果 |
|------|------|------|
| 召回 | Recall@5（生产全管线，语料 10→155 文档仅降 0.4pt） | **0.989** |
| 抗误召回 | 负样本误召回率（重排严格口径） | **11.5%** → 阈值调优后 **5.1%** |
| 忠实 | Faithfulness（回答论断被资料支持率） | **0.995** |
| 正确性 | Answer Correctness（对权威标准答案） | **0.938** |
| 上下文 | Context Recall（标准答案要点被检索覆盖） | **0.957** |
| 拒答 | 陷阱负样本正确拒答率（不编造） | **0.980** |
| 压力 | 10 并发混合格式入库 / 30 万字长文档 | **0 失败** · 99 项测试全过 |

<p align="center"><b>语料规模扩大 38 倍，召回仅降 0.4 个点；对抗性陷阱下"检索防线 + LLM 拒答"双层防线全部守住。</b></p>
...
指标结果见顶部「🏆 检索与回答质量」，四条命令全流程复现（数据集构建→入库→评测）：

```bash
# 构建评测集与评测库（824 条 / 155 文件，HF 镜像下载→转换→自检）
